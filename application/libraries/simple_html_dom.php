<?php
// 利用simple_html_dom dom類為CI修改的一個類庫,可以像JS一樣對HTML元素進行分析,適合與抓取網頁時,對網頁數據進行的分析。 

// 類庫下載地址: http://sourceforge.net/projects/simplehtmldom/ 

// 修改： 
// 把simple_html_dom批量替換為CI_Simple_html_dom。 
...
function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT)
{
    ...
    $dom = new CI_Simple_html_dom(null, $lowercase, $forceTagsClosed, $target_charset, $defaultBRText);
...
class CI_Simple_html_dom {
    ...
    function load_file() {
        ...
        // Per the CI_Simple_html_dom repositiry this is a planned upgrade to the codebase.
    ...
    protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT) {
        ...
        $this->root = new CI_Simple_html_dom_node($this);
        $this->root->tag = 'root';
    ...
    protected function parse() {
        ...
        // text
        $node = new CI_Simple_html_dom_node($this);
        ++$this->cursor;
    ...
    protected function read_tag() {
        ...
        if ($this->char==='/') {
            $this->char = (++$this->pos<$this->size) ? $this->doc[$this->pos] : null; // next
            // This represetns the change in the CI_Simple_html_dom trunk from revision 180 to 181.
