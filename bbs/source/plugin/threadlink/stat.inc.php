...
if($subop == 'xml') {
	...
	if($_GET['hash'] != $hash && $stat_hash != $_GET['hash']) {
	...
	$_GET['begin'] = dgmdate(dmktime($_GET['begin']), 'Y-m-d');
	$_GET['end'] = dgmdate(dmktime($_GET['end']), 'Y-m-d');
	...
	if(submitcheck('submit')) {
		$_GET['date_after'] = dgmdate(dmktime($_GET['date_after']), 'Y-m-d');
		$_GET['date_before'] = dgmdate(dmktime($_GET['date_before']), 'Y-m-d');
		$begin = !empty($_GET['date_after']) ? $_GET['date_after'] : dgmdate($_G['timestamp']-604800, 'Y-m-d');
		$end = !empty($_GET['date_before']) ? $_GET['date_before'] : dgmdate($_G['timestamp'], 'Y-m-d');
		if($begin > $end) {
	...
	$option = 0;
	if($_GET['create_base']) $option += 1;
	if($_GET['create_link']) $option += 2;
	if($_GET['modify_link']) $option += 4;
	if($_GET['clicks']) $option += 8;
	showoption();
	if(submitcheck('submit')) {
