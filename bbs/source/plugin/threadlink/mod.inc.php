...
$action = in_array($_GET['action'], $allowaction) ? $_GET['action'] : '';
$tid = intval($_GET['tid']);
...
//聚合到帖子内
	...
	if(!submitcheck('addlinksubmit')) {
		...
		if($pid) {
			...
			$linkthread['message'] = messagecutstr(dhtmlspecialchars($linkthread['message']), $thread_count['summarylength']);
			//获取图片
		...
		$pid = intval($_GET['pid']);
		$subject = dhtmlspecialchars(daddslashes(cutstr(trim($_GET['subject']), 80)));
		$message = dhtmlspecialchars(daddslashes(cutstr(trim($_GET['message']), $thread_base['summarylength'])));
		$url = dhtmlspecialchars(daddslashes(trim($_GET['url'])));
		...
		$attach_hand = (array)explode(',', trim($_GET['attach_hand']));
		if(!empty($attach_hand)) {
			$i = 0;
			foreach($attach_hand as $inputname) {
				$i++;
				if(empty($inputname) || empty($_GET['pic'][$inputname])) {
					continue;
				}
				if(substr($_GET['pic'][$inputname], 0, 4) == 'aid:'){
					$aid_tmp[] = intval(str_replace('aid:', '', $_GET['pic'][$inputname]));
				}
				if($i >= 4) {
					break;
				}
	...
	if($link) {
		if(!submitcheck('editsubmit')) {
			...
			$tid = !$_GET['change_base'] ? $link['tid'] : intval($_GET['tid_hand']);
			$thread_base = DB::fetch_first("SELECT * FROM ".DB::table('threadlink_base')." WHERE tid='$tid'");
			$subject = dhtmlspecialchars(daddslashes(cutstr(trim($_GET['subject']), 80)));
			$message = dhtmlspecialchars(daddslashes(cutstr(trim($_GET['message']), 200)));
			$url = dhtmlspecialchars(daddslashes(trim($_GET['url'])));
			$pic = dhtmlspecialchars(daddslashes(trim($_G['pic'])));
	...
	$message = '';
	$post = get_post_by_pid($_GET['pid'], 'pid,tid,attachment');
	...
	if(empty($lid)) {
		...
		if(!submitcheck('editimagesubmit')) {
			...
			$picurl = dhtmlspecialchars(daddslashes(trim($_GET['picurl'])));
			if(empty($picurl)) {
	...
	if(empty($tid)) {
		...
		if(DB::result_first("SELECT COUNT(*) FROM ".DB::table('threadlink_base')." WHERE tid='$tid'")) {
			if(!submitcheck('sortsubmit')) {
				...
				if(empty($_GET['displayorder']) || !is_array($_GET['displayorder'])) {
					showmessage(pl('displayorder_wrong'));
				} else {
					$sql = '';
					foreach($_GET['displayorder'] as $lid => $order) {
						$lid = intval($lid);
