...
<!-- mw_syntaxhighlighter ver.".$this->libversion." End -->\n";
	...
	function _mw_shl_code_to_pre($brush, $gutter, $code) {
		...
		return '<pre class="brush: '.strtolower($brush).'; gutter: '.$gutter.'">'.$code.'</pre>';
	}
...
class plugin_mw_syntaxhighlighter_forum extends plugin_mw_syntaxhighlighter {
	...
	function viewthread_bottom_output() {
		...
		foreach($postlist as $pid => $post) {
			$post['message'] = preg_replace('/<pre[^>](.*?)>(.*?)<\/pre>/ies', "\$this->_brreplace('\\1', '\\2')", $post['message']);
			$postlist[$pid] = $post;
		}
		return '';
	}

	function _brreplace($param, $codecontent) {
		return '<pre '.stripslashes($param).'>'.str_replace(array('<br>', '<br />'), '', $codecontent).'</pre>';
	}
