...
if(!$act) {
	showmessage(do_iconv('未定义操作，请返回。'), 'index.php');
}

$fb_cf = $_G['cache']['plugin']['facebook_connect'];
if(!$fb_cf['appid'] || !$fb_cf['secret']) {
	showmessage(do_iconv('请先到后台设置Facebook Connect插件，输入正确的FB应用程式ID与密钥。'), null, array(), array('alert'=>'error'));
}
...
if($act == 'logging') {
	if($_G['uid']) {
		showmessage(do_iconv('您已经登入！'), 'index.php', array(), array('alert'=>'info'));
	} else {
	...
	if($fb_me['id'] && $fb_me['name'] && $fb_me['email']) {
		...
		if($fcuid['uid'] || $cmuid['uid']) {
			...
			showmessage(do_iconv('您透过Facebook帐户登入成功<br><br>欢迎回访：').$_G['member']['username'], urldecode($rf), array(), array('alert'=>'right'));
		} else {
