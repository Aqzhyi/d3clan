...
<div class="container">
	<h3 class="marginbot">
		...
		通知列表
		<?php if($user['allowadminlog'] || $user['isfounder']) { ?><a href="admin.php?m=log&a=ls" class="sgbtn">日誌列表</a><?php } ?>
		<a href="admin.php?m=mail&a=ls" class="sgbtn">郵件隊列</a>
	</h3>
	<?php if($status == 2) { ?>
	...
	<div class="mainbox">
		<?php if($notelist) { ?>
			...
			<table class="datalist" onmouseover="addMouseEvent(this);" style="table-layout:fixed">
				<tr>
					<th width="60"><input type="checkbox" name="chkall" id="chkall" onclick="checkall('delete[]')" class="checkbox" /><label for="chkall">刪除</label></th>
					<th width="130">操作</th>
					<th width="60">通知次數</th>
					<th width="50">參數</th>
					<th width="140">最後通知時間</th>
