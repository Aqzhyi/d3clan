 ...
 *
 */
function obj_count(objs)
{
	var n = 0;
	for(var obj in objs) n++;
	return n;
}
var Tooltip = {
     ...
     */
    initialize: function() {
		var tooltipDiv = $('<div/>').addClass('ui-tooltip').appendTo("body").attr("id","ui-tooltip").attr("tabindex","2");
	...
	bind: function(query, options, callback) {
		...
		doc.undelegate(query, 'mouseenter', func);
		doc.delegate(query, 'mouseenter', func);
	},
		bind2: function(query, options, callback) {
		var doc = $(document),
			func;
		...
		if (Core.isCallback(callback)) {
			...
			func = function() {
				this.focus();
				var self = $(this),
				title = self.data('tooltips') || this.title;
		...
		doc.undelegate(query, 'click', func);
		doc.delegate(query, 'click', func);
	},
    ...
    show: function(node, content, options) {
		...
		Tooltip.position(node, "<span><b>loading...</b></span>", {location:'topLeft'});
		// Update trigger node
		...
		if (typeof content === 'object') {
			...
			// Content: AJAX
			if (options.ajax) {
				content = $(node).attr("tip-slug");
				if (Tooltip.cache[content]) {
					Tooltip.position(node, Tooltip.cache[content], options);
				} else {
					if(Tooltip.cache_tmp[content] != undefined) return;
					var urls = $(node).attr("tip-url").split("|");
					var slugs = $(node).attr("tip-slug").replace(/-/g,"_").split("|");
					if(Tooltip.cache_tmp[content] == undefined) Tooltip.cache_tmp[content] = "";
					if(Tooltip.cache_init[content] == undefined) Tooltip.cache_init[content] = {};
					for(var i = 0;i < urls.length;i++)
					{
						$.ajax({
							...
							dataType: "jsonp",
							global: false,
							jsonpCallback:'callback_'+slugs[i],
							success: function(data) {
								if(Tooltip.cache_init[content][data.params.type] != undefined) return;
								Tooltip.cache_init[content][data.params.type] = data.tooltipHtml;
								if(obj_count(Tooltip.cache_init[content]) == urls.length){
									if(urls.length > 1){
										Tooltip.cache[content] = Tooltip.cache_init[content]["skill"] + Tooltip.cache_init[content]["rune"];
									}else{
										Tooltip.cache[content] = Tooltip.cache_init[content]["skill"];
									}
							...
							error: function(xhr,a,b) {
								Tooltip.cache_tmp[content] = undefined;
								if (xhr.status != 200)
									Tooltip.hide();
							}
						});
	...
	D3:{
		Tooltips:{
			registerData:function(data){
				eval("callback_"+data.params.key.replace(/-/g,"_")+"(data)");
			}
		}
	}	
}
